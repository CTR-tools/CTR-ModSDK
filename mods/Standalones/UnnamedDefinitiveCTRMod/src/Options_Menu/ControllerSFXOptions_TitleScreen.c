...
force_inline void DISPLAYRECTMENU_MainFreeze_MenuPtrOptions(struct RectMenu* menu, GAMEPAD_MainFreeze_MenuPtrOptions* gamepad)
{
	...
	{
		// "CONFIGURE ANALOG:"
		DecalFont_DrawLine(sdata->lngStrings[336], 76 - UDCTRM_OM_ControllerSFXMenuBoxWidthPadding/2, analogRowPosY + (menuRowsNegativePadding / 2) + 90, FONT_SMALL, ORANGE);
	...
	// "EXIT"
	DecalFont_DrawLine(sdata->lngStrings[331], 76 - UDCTRM_OM_ControllerSFXMenuBoxWidthPadding/2, (menuRowsNegativePadding / 2) + 140 - menuRowsNegativePadding, FONT_SMALL, ORANGE);

	RECT cursor =
	{
		.x = 74 - UDCTRM_OM_ControllerSFXMenuBoxWidthPadding/2,
	...
	{
		.x = 66 - UDCTRM_OM_ControllerSFXMenuBoxWidthPadding/2,
	...
	RECT menuBoxBG =
	{
		.x = 56 - UDCTRM_OM_ControllerSFXMenuBoxWidthPadding/2,
