...
{
	u_int firstRowY = ((UDCTRM_OM_MenuBoxBG_y + 4) + 26);
	u_int optionTextPosX = (UDCTRM_OM_glowingcursor.x + UDCTRM_OM_glowingcursor.w) - 2;

	UDCTRM_OM_glowingcursor.y = firstRowY + (10 * mb->rowSelected);

	// "RACE OPTIONS"
	DecalFont_DrawLine(sdata->lngStrings[593], SCREEN_WIDTH/2, UDCTRM_OM_MenuBoxBG_y + 4, FONT_BIG, (JUSTIFY_CENTER | ORANGE));

	// "LAPS"
	DecalFont_DrawLine(sdata->lngStrings[594], UDCTRM_OM_glowingcursor.x + 2, firstRowY + (10 * 0) + 1, FONT_SMALL, ORANGE);
	// "BLUE FIRE"
	DecalFont_DrawLine(sdata->lngStrings[595], UDCTRM_OM_glowingcursor.x + 2, firstRowY + (10 * 1) + 1, FONT_SMALL, ORANGE);
	// "MIRROR MODE"
	DecalFont_DrawLine(sdata->lngStrings[596], UDCTRM_OM_glowingcursor.x + 2, firstRowY + (10 * 2) + 1, FONT_SMALL, ORANGE);
	// "EXIT"
	DecalFont_DrawLine(sdata->lngStrings[331], UDCTRM_OM_glowingcursor.x + 2, firstRowY + (10 * 3) + 1, FONT_SMALL, ORANGE);
	...
	MENUBOX_DrawInnerRect(&UDCTRM_OM_titleSeparatorLine, 4, (u_long *)(gGT->backBuffer->otMem).startPlusFour); // draw the line that's below the title
	CTR_Box_DrawClearBox(&UDCTRM_OM_glowingcursor, &sdata->menuRowHighlight_Normal, 1, (u_long *)(gGT->backBuffer->otMem).startPlusFour, &gGT->backBuffer->primMem); // draw glowing cursor
	MENUBOX_DrawInnerRect(&UDCTRM_OM_menuBoxBG, 4, (u_long *)(gGT->backBuffer->otMem).startPlusFour); // draw the actual menubox background
}
