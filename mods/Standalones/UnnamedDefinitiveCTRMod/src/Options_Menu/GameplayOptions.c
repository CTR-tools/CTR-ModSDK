...
force_inline void DisplayMenuBox(struct MenuBox* mb, struct GameTracker* gGT)
{
	...
	// "RACE OPTIONS"
	DecalFont_DrawLine(sdata->lngStrings[593], SCREEN_WIDTH/2, UDCTRM_OM_GameplayMenuBoxBG_y + 4, FONT_BIG, (JUSTIFY_CENTER | ORANGE));

	// "LAPS"
	DecalFont_DrawLine(sdata->lngStrings[594], UDCTRM_OM_Gameplayglowingcursor.x + 2, firstRowY + (10 * 0) + 1, FONT_SMALL, ORANGE);
	// "BLUE FIRE"
	DecalFont_DrawLine(sdata->lngStrings[595], UDCTRM_OM_Gameplayglowingcursor.x + 2, firstRowY + (10 * 1) + 1, FONT_SMALL, ORANGE);
	// "MIRROR MODE"
	DecalFont_DrawLine(sdata->lngStrings[596], UDCTRM_OM_Gameplayglowingcursor.x + 2, firstRowY + (10 * 2) + 1, FONT_SMALL, ORANGE);
	// "EXIT"
	DecalFont_DrawLine(sdata->lngStrings[331], UDCTRM_OM_Gameplayglowingcursor.x + 2, firstRowY + (10 * 3) + 1, FONT_SMALL, ORANGE);
...
// remember: this is a fake menubox. this doesn't make use of a menubox struct at all, outside of rowSelected
