...
void MENUBOX_DrawSelf(struct MenuBox *mb, int posX, int posY, int menuboxWidth)
{
    u_short textFlags;
    u_int state;
    int index;
...
LAB_80045e94:
    ...
    {
        menuboxHeight = 0;
        MENUBOX_GetHeight(mb, &menuboxHeight, 1);
    ...
    if ((mb->state & 0x10) != 0)
    {
        MENUBOX_DrawSelf(mb->ptrNextBox_InHierarchy,
                         posX + mb->posX_prev,
    ...
    MENUBOX_DrawFullRect(mb, &borders);
}
