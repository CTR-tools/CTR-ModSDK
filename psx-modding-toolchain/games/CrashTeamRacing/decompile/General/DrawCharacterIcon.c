...
void DECOMP_DrawCharacterIcon(struct Icon* icon, short posX, short posY, struct PrimMem* primMem, u_long* ot, u_char semitransparencyEnabled, short scale, u_int vcolorAndCode)
{
	short iVar6;
	short height;
	short width;
	...
	*(u_int*)&p->u2 = *(u_int*)&icon->X3;
	...
	if (semitransparencyEnabled != 0)
	{
		...
		p->tpage = p->tpage & 0xff9f | ((u_short)semitransparencyEnabled - 1) << 5;
		p->code = p->code | 2;
	}
