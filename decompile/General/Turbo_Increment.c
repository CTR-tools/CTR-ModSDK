...
void DECOMP_Turbo_Increment(struct Driver* driver, int reserves, u_int type, int fireLevel)
{
	...
	if(turboThread == 0)
	{
		...
		{
			driver->numTurbos = 1;
			if ((driver->numTurbosBackup < 1) && ((gGT->gameMode1 & END_OF_RACE) == 0))
			{
		...
		{
			driver->numTurbos++;
			if ((driver->numTurbosBackup < driver->numTurbos) && ((gGT->gameMode1 & END_OF_RACE) == 0))
			{
	...
	else
	{
		...
		if (type & 4)
		{
			...
			if ((driver->actionsFlagSetPrevFrame & 0x200000) == 0)
			{
				...
				if (driver->numTurbosBackup < driver->numTurbos && (gGT->gameMode1 & END_OF_RACE) == 0) driver->numTurbosBackup = driver->numTurbos;
				#endif
			}
		}
