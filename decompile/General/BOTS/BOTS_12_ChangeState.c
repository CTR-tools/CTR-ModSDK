...
u_int DECOMP_BOTS_ChangeState(struct Driver* driverVictim, int damageType, struct Driver* driverAttacker, int reason)
{
	...
	case 4:
		if ((driverVictim->botFlags & 2) == 0)
		{
			driverVictim->unk5ba = 1;
			*((short*)&driverVictim->unk5bc[0x8]) = 0;
			...
			if ((data.characterIDs[driverVictim->driverID] != 0xf) || ((driverVictim->actionsFlagSet & 1) != 0))
			{
				...
				driverVictim->unk5bc[0x10] = 0;

				int newSpeed;

				if (data.characterIDs[driverVictim->driverID] == 0xf)
				{
					newSpeed = (*(int*)(&driverVictim->unk5bc[0x18])) >> 1;
				}
				else
				{
					newSpeed = (*(int*)(&driverVictim->unk5bc[0x18])) >> 2;
				}

				*(int*)(&driverVictim->unk5bc[0x18]) = newSpeed;
			}

			*(int*)(&driverVictim->unk5bc[0xc]) = 0x300;
			driverVictim->ai_progress_cooldown = 0;

			driverVictim->botFlags |= 2;
		}
		...
		break;
	case 2:
		driverVictim->unk626 = 0; //OG code just assigns 0 to 1 byte at +0x626, not 2 bytes
		(*(short*)&driverVictim->unk5bc[0x8]) = 0;
		driverVictim->unk5ba = 2;
		driverVictim->reserves = 0;
		driverVictim->turbo_outsideTimer = 0;
		(*(int*)&driverVictim->unk5bc[0x10]) = 0;
		(*(int*)&driverVictim->unk5bc[0x14]) = sdata->AI_VelY_WhenBlasted_0x3000;

		if ((driverVictim->botFlags & 2) == 0)
		{
			(*(int*)&driverVictim->unk5bc[0x18]) >>= 3;
			driverVictim->ai_posBackup[1] += 0x4000;
		}
	...
	case 3:
		*(short*)&driverVictim->unk5bc[0x8] = 0;
		...
		if (driverVictim->instSelf->thread->modelIndex == 0x18 && (*(int*)&driverVictim->unk5bc[0xC]) == 0)
		{
			OtherFX_Play(0x5a, 1);
		}

		driverVictim->unk5ba = 3;
		*(int*)&driverVictim->unk5bc[0xC] = 0x300;
		*(short*)&driverVictim->unk5bc[0x0] = 0xf00;
		driverVictim->squishTimer = 0xf00;
		driverVictim->reserves = 0;
		driverVictim->turbo_outsideTimer = 0;
		*(int*)&driverVictim->unk5bc[0x10] = 0;
		*(int*)&driverVictim->unk5bc[0x14] = 0;
		driverVictim->ai_progress_cooldown = 0;
		*(int*)&driverVictim->unk5bc[0x18] >>= 1;
		driverVictim->botFlags |= 6;
		break;
	case 5:
		*(short*)&driverVictim->unk5bc[0x8] = 0;
		*(int*)&driverVictim->unk5bc[0x18] = 0;
		*(int*)&driverVictim->unk5bc[0x14] = 0;
		*(int*)&driverVictim->unk5bc[0x10] = 0;
		driverVictim->instSelf->flags |= 0x80;
		*(short*)&driverVictim->unk5bc[0x0] = 0xd20;
		driverVictim->unk5ba = 5;
		driverVictim->kartState = KS_MASK_GRABBED;
		driverVictim->botFlags |= 6;
		driverVictim->instSelf->thread->flags |= 0x1000;
		break;
	default:
		driverVictim->ai_progress_cooldown = 0x3c;
	}
