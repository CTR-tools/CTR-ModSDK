...
void DECOMP_VehPtr_Drifting_PhysAngular(struct Thread* th, struct Driver* driver)
{
	...
	iVar13 = DECOMP_VehMath_MapToRange(iVar13, 0, ((u_int)driver->const_TurnRate + ((int)driver->turnConst << 1) / 5) * 0x100, 0, iVar9 << 8);
...
LAB_80063244:
		...
		driver->KartStates.Drifting.numFramesDrifting =
			DECOMP_VehMath_InterpBySpeed((int)driver->KartStates.Drifting.numFramesDrifting, 1, 0);
	}
	...
	iVar13 = DECOMP_VehMath_MapToRange((int)driver->KartStates.Drifting.driftTotalTimeMS, 0, (u_int)driver->unk462 << 5, (int)driver->unk461 * (int)driver->multDrift >> 8, iVar15);
	if (-1 < iVar13)
	{
		if (iVar12_D < -iVar13)
		{
...
LAB_800632cc:
	...
	// Map value from [oldMin, oldMax] to [newMin, newMax]
	// inverting newMin and newMax will give an inverse range mapping
	iVar12_D = DECOMP_VehMath_MapToRange(iVar12_D, 0, (int)driver->unk460 + ((int)driver->turnConst << 2) / 5, 0, (int)driver->unk474);
	...
	// Map value from [oldMin, oldMax] to [newMin, newMax]
	// inverting newMin and newMax will give an inverse range mapping
	iVar15 = DECOMP_VehMath_MapToRange(iVar15, 0, iVar11 << 8, 0, iVar8);
	...
	if (driver->unk3D4[2] == 0)
	{
		...
		// Interpolate rotation by speed
		sVar5 = DECOMP_VehMath_InterpBySpeed(driver->unk3D4[0], iVar12_E, 0);
	}
	else
	{
		driver->unk3D4[2]--;
    ...
    else
    {
        ...
        if (meterLeft != 0)
        {
            ...
            if (meterLeft < highMeter)
            {
                ...
                incrementReserves = DECOMP_VehMath_MapToRange(meterLeft, 0, highMeter, d->const_turboFullBarReserveGain << 5, 0);
