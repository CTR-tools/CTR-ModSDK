...
void DECOMP_Player_JumpAndFriction(struct Thread *t, struct Driver *d)
{
  char bVar1;
  int iVar2;
  int iVar3;
  ...
  CHECK_FOR_ANY_JUMP:
        ...
        (d->heldItemID == 5))
    {
      ...
      if ((d->jump_CoyoteTimerMS != 0) && (d->jump_CooldownMS == 0))
      {
        ...
        // spring weapon sound
        OtherFX_Play_Echo(9, 1, d->actionsFlagSet & 1);
    ...
    if (d->forcedJump_trampoline == 0)
    {
      ...
      // play jump sound
      OtherFX_Play_Echo(8, 1, (d->actionsFlagSet & 1 != 0));
    }
...
PROCESS_JUMP:
  ...
  int ramp = Player_Jump_GetVelY(d->AxisAngle4_normalVec[0], &movement);
  ...
  int jump = Player_Jump_GetVelY(axisAngle, &movement);
...
NOT_JUMPING:
  ...
  Driver_ConvertVectorsToSpeed(d, &movement);
