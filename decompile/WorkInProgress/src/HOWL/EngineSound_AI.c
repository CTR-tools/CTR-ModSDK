...
void DECOMP_EngineSound_AI(struct Driver *param_1, struct Driver *param_2, int param_3, int param_4, int param_5, uint leftRight)
{
  ...
  desiredVolume = MapToRange(param_1->fill_3B6[0], 0, param_1->const_SpeedometerScale_ClassStat, 0x82, 0xe6);
  
  if (param_4 < 2000 && 200 < param_4)
  {
    // Map value from [oldMin, oldMax] to [newMin, newMax]
    // inverting newMin and newMax will give an inverse range mapping
    desiredVolume = MapToRange(param_4, 200, 2000, desiredVolume, 0);
  }
  else
  {
    desiredVolume = 0;
  ...
  // Map value from [oldMin, oldMax] to [newMin, newMax]
  // inverting newMin and newMax will give an inverse range mapping
  speedoScale = MapToRange(param_1->fill_3B6[1], 0, param_1->const_SpeedometerScale_ClassStat, 0x3c, 0xaa);
  param_5 = param_5 >> 3;
