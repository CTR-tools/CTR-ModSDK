...
void EngineSound_Player(struct Driver *driver)
{
    ...
    if (driver->unk47B == 0)
    {
        ...
        vol = DECOMP_VehCalc_MapToRange(driver->fill_3B6[0], 0, driver->const_SpeedometerScale_ClassStat, 0, 0xe6);
    ...
    else
    {
        if (driver->unk47B == 1)
        {
            ...
            // Map value from [oldMin, oldMax] to [newMin, newMax]
            // inverting newMin and newMax will give an inverse range mapping
            vol = DECOMP_VehCalc_MapToRange(driver->fill_3B6[0], 0, driver->const_SpeedometerScale_ClassStat, 0x82, 0xe6);
