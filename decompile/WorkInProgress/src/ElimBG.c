...
void DECOMP_ElimBG_ToggleAllInstances(struct Instance* i, char boolGameIsPaused)
{
	int iVar1;
	...
	for (; uVar2 < uVar3; uVar2 += 0x40)
	{
		if (*(struct Instance **)(uVar2 + 0x2c) != 0)
		{
			DECOMP_ElimBG_ToggleInstance(*(struct Instance **)(uVar2 + 0x2c), boolGameIsPaused);
		}
	}
	for
	(
