...
void VehParticle_DriverMain(struct Thread *t, struct Driver *d, undefined *param_3, uint param_4)
{
  byte bVar1;
  short sVar2;
  short sVar3;
  ...
  if (isPlayer)
  {
    ...
    vol = DECOMP_VehMath_MapToRange(approxSpeed, 0, 5000, 0, 200);

    // distortion
    // Map value from [minSpeed, maxSpeed] to [minDistort, maxDistort]
    distort = DECOMP_VehMath_MapToRange(approxSpeed, 0, 12000, 0x6c, 0xd2);
    distort = iVar11 << 8;
  ...
  if (gGT->numPlyrCurrGame < 2)
  {
    ...
    if (((terrainFlags & 0x40) != 0) && ((d->actionsFlagSet & 2) != 0))
    {
      ...
      if (0x600 < approxSpeed)
      {
        ...
        {
          Driver_SetMatrix(m);
          VehParticle_Sparks_Ground(d, &data.emSet_GroundSparks[0]);
        }
      }
    }
    ...
    if (terrain->em_EvenFrame)
    {
      ...
      Driver_SetMatrix(m);
      VehParticle_Terrain_Ground(d, emitter);
    }
    ...
    {
      Driver_SetMatrix(m);
      VehParticle_Sparks_Wall(d, &data.emSet_WallSparks[0]);
  ...
  else
  {
    ...
    if (isPlayer)
    {
      ...
      iVar11 = DECOMP_VehMath_MapToRange(approxSpeed, 2000, 12000, 0x14, 0xaa);

      // Map value from [oldMin, oldMax] to [newMin, newMax]
      // inverting newMin and newMax will give an inverse range mapping
      iVar15 = DECOMP_VehMath_MapToRange(approxSpeed, 2000, 12000, 0x92, 0x78);
  ...
  *(undefined **)(puVar21 + 0x30) = puVar12;
  ...
  VehParticle_Exhaust(d, r0, puVar21);
