...
void DECOMP_LoadSave_DrawAdvProfile(struct AdvProgress *adv, int posX, int posY, u_int isHighlighted, short slotIndex, u_short menuFlag)
{
    ...
    else
    {
        ...
        // Character Icon
        MENUBOX_DrawPolyGT4(gGT->ptrIcons[data.MetaDataCharacters[adv->characterID].iconID],
                            posX + 10, posY + 6,
        ...
        // Print the numbers
        LoadSave_PrintInteger(gGT->currAdvProfile.completionPercent, posX + 0x6a, posY + 23, 0, integerColor);
        LoadSave_PrintInteger(gGT->currAdvProfile.numTrophies, posX + 0x6a, posY + 5, 0, integerColor);
        LoadSave_PrintInteger(gGT->currAdvProfile.numKeys, posX + 0xb5, posY + 5, 0, integerColor);
        LoadSave_PrintInteger(gGT->currAdvProfile.numRelics, posX + 0xb5, posY + 23, 0, integerColor);
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            inst->matrix.t[0] = LoadSave_UI_ConvertX(posX + (i == 1 ? 0x78 : 0xc3), 0x100);
            inst->matrix.t[1] = LoadSave_UI_ConvertY(posY + (i == 0 ? 0x1f : 0xd), 0x100);
    ...
    if (isHighlighted != 0)
    {
        ...
        highlight.w = 208;
        highlight.h = 53;
        CTR_Box_DrawClearBox(&highlight, highlightColor, 1, gGT->backBuffer->otMem.startPlusFour[0xC], &gGT->backBuffer->primMem);
    }
    ...
    box.w = 220;
    box.h = 61;
    MENUBOX_DrawInnerRect(&box, menuFlag, gGT->backBuffer->otMem.startPlusFour[0xC]);
}
