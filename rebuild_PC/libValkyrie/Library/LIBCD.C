...
#include "Core/Setup/Game/EMULATOR_SETUP.H"
...
CdlCB readCallback = NULL;
CdlCB readyCallback = NULL;
CdlCB syncCallback = NULL;
...
struct AudioSector sectorBuff;
static int currentSectorBuffPos = 0;
...
int CdControl(u_char com, u_char * param, u_char * result)
{
	...
	case CdlReadN:
	{
		...
		fread(&sectorBuff, sizeof(struct AudioSector), 1, openFile);
		currentSectorBuffPos = 12;

		if (syncCallback != NULL)
		{
			syncCallback(CdlDataReady, result);
		}

		if(readyCallback != NULL)
		{
			readyCallback(CdlDataReady, result);
		}
...
{
	CdlCB lastCallback = readyCallback;
	readyCallback = func;
	return lastCallback;
}
...
{
	CdlCB lastCallback = syncCallback;
	syncCallback = func;
	return lastCallback;
}
...
int CdGetSector(void* madr, int size)
{
	if (openFile != NULL)
	{
		//int currentPos = ftell(openFile);
#if 1
		memcpy(madr, &sectorBuff.sync[currentSectorBuffPos], size << 2);
		currentSectorBuffPos += size << 2;
#else
		struct AudioSector sector;
		fread(&sector, sizeof(AudioSector), 1, openFile);
		memcpy(madr, &sectorBuff.sync[currentSectorBuffPos], size << 2);
#endif
		//fseek(openFile, currentPos, SEEK_SET);
	}
	return 0;
}
